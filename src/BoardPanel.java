...
class BoardPanel extends JPanel {
    ...
    // pion image per player
    private Map<Player, Image> playerSprites = new HashMap<>();
    ...
    private void loadPlayerSprites() {
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            String fileName = "player" + (i + 1) + ".png";

            try {
                Image img = ImageIO.read(new File(fileName));
                playerSprites.put(p, img);
            } catch (IOException e) {
                System.err.println("Missing sprite: " + fileName);
            }
        }
    }
    ...
    private void drawPlayers(Graphics2D g2d) {
        ...
        for (var entry : map.entrySet()) {
            ...
            Point base = getPixelPosition(pos);

            for (int i = 0; i < list.size(); i++) {
                Player p = list.get(i);
                Image sprite = playerSprites.get(p);
                ...
                int x = base.x + offsetX;
                int y = base.y + offsetY;

                if (sprite != null) {
                    g2d.drawImage(sprite, x - 20, y - 20, 40, 40, this);
                } else {
                    // fallback circle
    ...
    private Point getPixelPosition(int logicalPos) {
        ...
        Point2D.Double rel = nodeLocations[logicalPos];

        if (rel == null) {
            System.err.println("⚠ Missing node location for " + logicalPos);
            rel = nodeLocations[1]; // fallback ke START
        }

        int x = (int) (rel.x * getWidth());
        int y = (int) (rel.y * getHeight());
        return new Point(x, y);
    }

    // === NODE LOCATIONS (TIDAK DIUBAH)
    private void initNodeLocations() {
        nodeLocations = new Point2D.Double[75];
