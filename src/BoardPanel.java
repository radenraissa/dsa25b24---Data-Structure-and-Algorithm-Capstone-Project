...
class BoardPanel extends JPanel {
    ...
    private void drawPlayers(Graphics2D g2d) {
        ...
        for (Player p : players) {
            playersByPos.computeIfAbsent(p.getPosition(), k -> new ArrayList<>()).add(p);
        }

        for (Map.Entry<Integer, List<Player>> entry : playersByPos.entrySet()) {
            Point base = getPixelPosition(entry.getKey());
            List<Player> sameCell = entry.getValue();

            for (int i = 0; i < sameCell.size(); i++) {
                Player p = sameCell.get(i);

                int offsetX = 0, offsetY = 0;
                if (sameCell.size() > 1) {
                    double angle = 2 * Math.PI * i / sameCell.size();
                ...
                if (p.getPawnImage() != null) {
                    g2d.drawImage(p.getPawnImage(), x - 20, y - 20, 40, 40, this);
                } else {
    ...
    private Point getPixelPosition(int logicalPos) {
        ...
        Point2D.Double p = nodeLocations[logicalPos];
